...
export async function getMyExpertProfile(userId) {
  ...
  const [{ data: expert }, { data: profile }, { data: cats }] = await Promise.all([
    ...
    supabase.from("profiles").select("id, full_name, avatar_initials, avatar_color, avatar_url, role").eq("id", userId).maybeSingle(),
...
export async function saveExpertProfile(userId, f) {
  ...
  const { error: eErr } = await supabase.from("experts").update({
    ...
    pitch_mn: f.pitch_mn || "",
    pitch_en: f.pitch_en || "",
    ideal_for_mn: (f.ideal_for_mn || []).map((x) => (x || "").trim()).filter(Boolean),
    ideal_for_en: (f.ideal_for_en || []).map((x) => (x || "").trim()).filter(Boolean),
...
export const PROFILE_STEPS = [
  { key: "full_name",   points: 8,  test: (f) => (f.full_name || "").trim().length > 1 },
  { key: "photo",       points: 12, test: (f) => !!f.avatar_url },
  { key: "pitch",       points: 12, test: (f) => (f.pitch_mn || "").trim().length > 15 },
  { key: "ideal_for",   points: 8,  test: (f) => (f.ideal_for_mn || []).filter((x) => (x || "").trim()).length >= 2 },
  { key: "headline",    points: 14, test: (f) => (f.headline_mn || "").trim().length > 5
                                                 && !(f.headline_mn || "").includes("шинэ мэргэжилтэн") },
  { key: "bio",         points: 14, test: (f) => (f.bio_mn || "").trim().length >= 80 },
  { key: "skills",      points: 14, test: (f) => (f.skills || []).length >= 3 },
  { key: "categories",  points: 8, test: (f) => (f.categories || []).length >= 1 },
  { key: "fee",         points: 5, test: (f) => Number(f.fee_mnt) > 0 },
  { key: "credentials", points: 5, test: (f) => (f.credentials_mn || "").trim().length > 5 },
];
...
export function nextBestAction(f) {
  ...
  const order = ["photo", "pitch", "headline", "ideal_for", "bio", "skills", "categories", "credentials", "fee", "full_name"];
