...
export const PROJECT_SORTS = ["relevance", "new", "needs", "team", "popular"];
...
export async function searchProjects(f = {}, { limit = 24, offset = 0 } = {}) {
  const { data, error } = await supabase.rpc("search_projects", {
    ...
    p_open_roles: f.openRoles ? true : null,
    p_small_team: f.smallTeam ? true : null,
...
export function projectFiltersFromParams(sp) {
  ...
  return {
    ...
    statuses: csv("status"),
    lookingFor: csv("role"),
    openRoles: sp.get("roles") === "1",
    smallTeam: sp.get("small") === "1",
...
export function projectParamsFromFilters(f) {
  ...
  if (f.openRoles) o.roles = "1";
  if (f.smallTeam) o.small = "1";
...
export function countProjectFilters(f) {
  return (f.categories?.length || 0) + (f.tags?.length || 0) +
    ...
    (f.openRoles ? 1 : 0) + (f.smallTeam ? 1 : 0);
}
