...
export default function ProjectSearchBar({ filters, onChange, total, onOpenFilters }) {
  ...
  const pills = [
    ...
    ...(filters.needsFunding ? [{ label: t("pj_needs_funding"), clear: () => set({ needsFunding: false }) }] : []),
    ...(filters.hasGoal ? [{ label: t("pj_has_goal"), clear: () => set({ hasGoal: false }) }] : []),
  ];
      ...
      <div className="exbar">
        ...
        <div className="exbar-right">
          ...
          <select className="pselect" value={filters.sort || "relevance"} onChange={(e) => set({ sort: e.target.value })}>
            ...
            <option value="progress">{t("pj_sort_progress")}</option>
            <option value="funded">{t("pj_sort_funded")}</option>
