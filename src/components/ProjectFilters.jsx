...
export default function ProjectFilters({ filters, onChange, open, onClose }) {
  ...
  const body = (
    <>
      ...
      <Section title={t("pj_tab_funding")}>
        <label className="facet-row">
          <input type="checkbox" checked={!!filters.needsFunding}
            onChange={(e) => set({ needsFunding: e.target.checked })} />
          <span className="facet-label">💰 {t("pj_needs_funding")}</span>
        </label>
        <label className="facet-row">
          <input type="checkbox" checked={!!filters.hasGoal}
            onChange={(e) => set({ hasGoal: e.target.checked })} />
          <span className="facet-label">🎯 {t("pj_has_goal")}</span>
        </label>
        <p className="facet-note">{t("pj_funding_note")}</p>
      </Section>
